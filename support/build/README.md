...
From this, the buildpack would create a "platform package" `.heroku/php/composer.json` like the following, with the main [packagist.org](https://packagist.org) repository disabled, and a few custom repositories as well as static package definitions added:

    {
    	...
    	"repositories": [
    		...
    		{
    			"type": "composer",
    			"url": "https://magalu-php-buildpack.s3.amazonaws.com/dist-heroku-18-stable/"
    		},
    		{
    			"type": "package",
    			"package": [
...
- `STACK` (currently, only "`heroku-16`" or "`heroku-18`" make any sense)

The following environment variables are highly recommended (see section *Understanding Upstream Buckets*):

- `UPSTREAM_S3_BUCKET` where dependencies are pulled from if they can't be found in `S3_BUCKET+S3_PREFIX`, should probably be set to "`magalu-php-buildpack`", the official Heroku bucket
...
That's where the `UPSTREAM_S3_BUCKET` and `UPSTREAM_S3_PREFIX` env vars documented above come into play; you'll usually set them to "`magalu-php-buildpack`" and "`dist-heroku-18-stable/`", respectively (or whatever stack you're trying to build for).
...
A manifest looks roughly like this (example is for `ext-apcu/5.1.17` for PHP 7.3 on stack `heroku-18`):

    {
    	...
    	"dist": {
    		"type": "heroku-sys-tar",
    		"url": "https://magalu-php-buildpack.s3.amazonaws.com/dist-heroku-18-stable/extensions/no-debug-non-zts-20180731/apcu-5.1.17.tar.gz"
    	},
...
*Example: `curl -s https://magalu-php-buildpack.s3.amazonaws.com/dist-heroku-18-stable/packages.json | jq '[ .packages[][] | select(.type == "heroku-sys-php-extension" and .name == "heroku-sys/ext-apcu") ] | .[0]'`*
...
*Example: `curl -s https://magalu-php-buildpack.s3.amazonaws.com/dist-heroku-18-stable/packages.json | jq '[ .packages[][] | select(.type == "heroku-sys-php") ][0] | {require}'`*
...
*Example: `curl -s https://magalu-php-buildpack.s3.amazonaws.com/dist-heroku-18-stable/packages.json | jq '[ .packages[][] | select(.type == "heroku-sys-php-extension") ][0] | {require}'`*
...
*Example: `curl -s https://magalu-php-buildpack.s3.amazonaws.com/dist-heroku-18-stable/packages.json | jq '[ .packages[][] | select(.name == "heroku-sys/ext-pq") ][0] | {require}'`*
...
*Example: `curl -s https://magalu-php-buildpack.s3.amazonaws.com/dist-heroku-18-stable/packages.json | jq '[ .packages[][] | select(.type == "heroku-sys-php") ][0] | {dist}'`*
...
*Example: `curl -s https://magalu-php-buildpack.s3.amazonaws.com/dist-heroku-18-stable/packages.json | jq '[ .packages[][] | select(.type == "heroku-sys-php") ][0] | {replace}'`*
...
*Example: `curl -s https://magalu-php-buildpack.s3.amazonaws.com/dist-heroku-18-stable/packages.json | jq '[ .packages[][] | select(.name == "heroku-sys/ext-newrelic") ][0] | {extra: {config: .extra.config}}'`*
...
*Example: `curl -s https://magalu-php-buildpack.s3.amazonaws.com/dist-heroku-18-stable/packages.json | jq '[ .packages[][] | select(.type == "heroku-sys-webserver") ][0] | {extra: {export: .extra.export}}'`*
...
*Example: `curl -s https://magalu-php-buildpack.s3.amazonaws.com/dist-heroku-18-stable/packages.json | jq '[ .packages[][] | select(.type == "heroku-sys-webserver") ][0] | {extra: {profile: .extra.profile}}'`*
...
*Example: `curl -s https://magalu-php-buildpack.s3.amazonaws.com/dist-heroku-18-stable/packages.json | jq '[ .packages[][] | select(.name == "heroku-sys/ext-newrelic") ][0] | {extra: {profile: .extra.profile}}'`*
...
*Example: `curl -s https://magalu-php-buildpack.s3.amazonaws.com/dist-heroku-18-stable/packages.json | jq '[ .packages[][] | select(.type == "heroku-sys-php") ][0] | {extra: {shared: .extra.shared}}'`*
...
Create a `heroku-18.Dockerfile` with the following contents:
    ...
    ENV UPSTREAM_S3_BUCKET=magalu-php-buildpack
